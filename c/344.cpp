...
ll a[210000], b[210000], n, dp[210000], sum[210000], best;
...
{   
   ...
   fa(i,1,n+1)r(a[i]);
   ll netsum = 0;
   fa(i,1,n+1)netsum += (i) * a[i];
   fa(i,1,n+1) sum[i] = sum [i-1] + a[i];
   ConvexHullDynamic forr(1),bacrr(1);
   ll maxer = netsum;
   forr.addLine(a[1],sum[1]-1*a[1]);
   fa(i,2,n+1)
   {
     maxer = maX(maxer,netsum - sum[i] + forr.getBest(i));
     forr.addLine(a[i],sum[i]-i*a[i]);
   }
   bacrr.addLine(a[n],sum[n]-n*a[n]);
   fa(i,n,0)
   {
     maxer = maX(maxer,netsum - sum[i] + bacrr.getBest(i));
     bacrr.addLine(a[i],sum[i]-i*a[i]);
   }
